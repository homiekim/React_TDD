...
## Test 코드 작성원칙
- 테스트하고자 하는 코드의 기능이 변경되면, test code 역시 업데이트 해줘야함(유지 보수)
- 유지보수성을 높이기 위해 내부 구현 사항을 테스트 x
- 재사용성 높이기 (테스트용 함수나 class를 별도로 작성하여 재사용성을 높이는 것이 좋다)
- 배포시에 테스트 코드가 포함 되지 않도록 분리
- 테스트코드를 통한 문서화 (테스트 코드만 봐도 코드를 이해할 수 있도록)

### 테스트 구조?
- Before -> Test -> After 구조로 실행 됨
- Test 과정에는 준비(테스트에 필요한 object 나 data를 준비) -> 실행 -> 실행 된 값을 검증(`Given`, `When`, `Then`)
- 실행 과정에서 의도적으로 실패할 수 있어야 함

### FIRST 원칙
- F: `Fast` 느린것에 대한 의존성 낮추기 (파일, db, 네트워크 등 에 의존하는 테스트 코드는 -> mock ,stub 등을 활용해 의존성을 낮춘다)
- I: `Isolated` 최소한의 유닛으로 검증, 독릭적으로 집중적으로 테스트
- R: `Repeatable` 실행할 때마다 동일한 결과를 유지 (환경에 영향을 받지 않도록)
- S: `Self-Validating` 테스트 코드 내에서 스스로 결과를 검증 할 수 있도록 (jest 같은 라이브러리에서 검증할 수 있는 api를 제공, 수동적으로 값 비교x, ci/cd에 붙이면 검증을 자동화 가능)
- T: `Timely` 적절한 시기에 테스트 코드 작성 (사용자에게 배포되기 전에 테스트 코드를 통한 검증)

### 테스트 범위 원칙 (`Right-BICEP`)
- Right : 모든 요구 사항이 정상 동작하는지 확인
- B: `Boundary conditions` 모든 엣지 케이스에 대해 가능한 전부 테스트 함(큰 숫자, 작은 숫자, 잘못된 인풋, null값 등등)
- I: `Inverse relationship` 역관계를 적용해서 결과값을 확인
- C: `Cross check` 다른 수단을 이용해서 결과 값이 맞는지 확인
- E: `Error conditions` 에러에 대해 정삭적인 처리를 하는지 확인
- P: `Performance characteristics` 성능을 수치로 확인

### 테스트 커버리지(`CORRECT` 원칙)
- C: `Conformance` 특정한 포맷을 준수(전화 번호, 이메일, 아이디 등등)
- O: `Ordering` 순서가 중요한 경우 (ex- 정렬된 리스트를 받는 경우?) 순서가 정확하지 않을 때도 테스트 해봐야 함
- R: `Range` 범위, 기대하는 범위를 벗어난 경우를 테스트
- R: `Reference` 외부 의존성 유무, 특정한 조건의 유무 (특정한 상황일때~, 특정한 값일 때~ 이런 경우 테스트) 개발자가 가정하고 있는 상황이 아닐 때도 테스트
